...
class Scheduler:
    ...
    def __init__(
        ...
        self.ema_tput_merge = 0
        self.ema_tput_unmerge = 0
        self.ema_alpha = scheduler_config.alpha
    ...
    def _adjust_merge_threshold_ema(self):
        if self.num_iters == 0:
            return
        t_merge = self.merge_time * self.num_iters
        t_unmerge = self.unmerge_time * self.num_iters
        ...
        if self.ema_tput_merge == 0:
            self.ema_tput_merge = tput_merge
        else:
            self.ema_tput_merge = self.ema_alpha * tput_merge + (1 - self.ema_alpha) * self.ema_tput_merge
        if self.ema_tput_unmerge == 0:
            self.ema_tput_unmerge = tput_unmerge
        else:
            self.ema_tput_unmerge = self.ema_alpha * tput_unmerge + (1 - self.ema_alpha) * self.ema_tput_unmerge

        if self.merged:
            if self.ema_tput_merge > self.ema_tput_unmerge and self.merge_right_threshold - self.aimd_add > self.merge_left_threshold:
                self.merge_right_threshold -= self.aimd_add
                print("tput_merge", self.ema_tput_merge, 
                      "tput_unmerge", self.ema_tput_unmerge, 
                      "merge_right_threshold", self.merge_right_threshold)
            elif self.ema_tput_merge < self.ema_tput_unmerge:
                self.merge_right_threshold *= self.aimd_mul
                print("tput_merge", self.ema_tput_merge, 
                      "tput_unmerge", self.ema_tput_unmerge, 
                      "merge_right_threshold", self.merge_right_threshold)
        else:
            if self.ema_tput_unmerge > self.ema_tput_merge and self.merge_left_threshold + self.aimd_add < self.merge_right_threshold:
                self.merge_left_threshold += self.aimd_add
                print("tput_merge", self.ema_tput_merge, 
                      "tput_unmerge", self.ema_tput_unmerge, 
                      "merge_left_threshold", self.merge_left_threshold)
            elif self.ema_tput_unmerge < self.ema_tput_merge:
                self.merge_left_threshold /= self.aimd_mul
                print("tput_merge", self.ema_tput_merge, 
                      "tput_unmerge", self.ema_tput_unmerge, 
                      "merge_left_threshold", self.merge_left_threshold)
